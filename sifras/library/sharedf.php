...
function __autoload($className) {
    ...
    } else if (file_exists('library/htmlgen' . '/' . strtolower($className) . '.php')) {
        require_once('library/htmlgen' . '/' . strtolower($className) . '.php');
    } else {
		$filename = 'library/phpmailer/class.'.strtolower($className).'.php';
...
function egetUrl($c="", $a="", $pars=null){
	$url = 'index.php';
	if($c!= ""){
		$url = $url."?contr=".$c;
		if($a!=""){
...
function isLogged(){
	$u = new User();
	if(!$u->logged()){
		header("Location: ".getUrl('index','login'));
		die();
	}	
}
...
/** 
An example of user functions inclusion.
*/
if(file_exists("conf/localsharedf.php")){
	include "conf/localsharedf.php";
}
